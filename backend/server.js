...
// app.use(cors({ origin: ['https://shikhathekua.onrender.com'], credentials: true }));
...
const BACKEND_URL = 'https://shikhathekua.onrender.com'; // deployed backend URL

app.use(cors({ origin: ['https://shikhathekua.onrender.com','http://localhost:3000'], credentials: true }));
app.use(express.json());
...
const userSchema = new mongoose.Schema({
  ...
  orders: [{
    orderId: String,
    items: [{ id: String, name: String, price: Number, image: String, description: String, quantity: Number }],
    totalAmount: Number,
    orderDate: String,
    orderTime: String,
    status: String,
    customerDetails: { name: String, address: String, contact: String, location: String, paymentMethod: String }
  }],
  cart: [{ id: String, name: String, price: Number, image: String, description: String, quantity: Number }]
}, { timestamps: true });
...
(async () => {
  try {
    if ((await Admin.estimatedDocumentCount()) === 0) {
      await Admin.create({ username: 'superadmin', email: 'admin@thekua.com', password: 'Admin@123' });
      console.log('🔐 Default admin created');
    }
  } catch (e) { console.warn('⚠️ Admin creation failed', e); }
})();
...
const authenticateAdmin = (req, res, next) => {
  ...
  if (!token) return res.status(401).json({ message: 'Admin token required' });
  jwt.verify(token, JWT_SECRET, (err, admin) => {
    if (err || admin.role !== 'admin') return res.status(403).json({ message: 'Invalid token' });
    req.admin = admin;
    next();
  });
};
...
// Health check
app.get('/api/health', (req, res) => res.json({ message: 'Backend running 🚀', status: 'healthy' }));
...
app.post('/api/auth/signup', [
  ...
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ message: errors.array()[0]?.msg || 'Invalid input' });
  ...
  await user.save();
  const token = jwt.sign({ userId: user._id, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
  res.status(201).json({ message: 'User created', token, user: { _id: user._id, name: user.name, email: user.email } });
});
...
app.put('/api/user/cart', authenticateToken, async (req, res) => {
  const user = await User.findByIdAndUpdate(req.user.userId, { cart: req.body.cart || [] }, { new: true }).select('cart');
  res.json({ message: 'Cart updated', cart: user.cart });
});
...
app.post('/api/user/orders', authenticateToken, async (req, res) => {
  const { orderData } = req.body;
  const user = await User.findById(req.user.userId);
  const newOrder = await Order.create({ ...orderData, user: user._id });
  user.cart = [];
  await user.save();
  res.status(201).json({ message: 'Order placed', order: newOrder });
});
