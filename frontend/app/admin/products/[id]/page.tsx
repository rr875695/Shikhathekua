...
export default function ProductFormPage() {
  ...
  const [product, setProduct] = useState<Product>({
    ...
    price: "",
    description: "",
    image: "/sugar.jpeg" // default image
  })
  const [saving, setSaving] = useState(false)
  const [uploading, setUploading] = useState(false)
  const token = typeof window !== "undefined" ? localStorage.getItem("adminToken") : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const body = { ...product, price: Number(product.price) }
    setSaving(true)
    try {
      if (!token) {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.message || "Upload failed")
      setProduct((prev) => ({ ...prev, image: data.url || "/sugar.jpeg" }))
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              />
              {uploading && <span className="text-sm text-slate-600">Uploading...</span>}
              {!uploading && product.image && <span className="text-sm text-emerald-600">Uploaded</span>}
            </div>
          </div>
          <div>
            <label className="block text-sm mb-1">Description</label>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => router.push("/admin/products")} className="admin-btn secondary">
              Cancel
            </button>
          </div>
        </form>
